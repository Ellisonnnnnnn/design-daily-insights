...
```bash
# Clone 到你的 skills 目录
git clone https://github.com/yourusername/design-digest ~/.openclaw/workspace/skills/design-digest
```
...
```bash
# 每天早上 8:30 推送
openclaw cron add --cron "30 8 * * *" --message "/design" \
  --session-key agent:main:feishu:direct:YOUR_CHAT_ID \
  --name "Design Daily Digest" \
  --tz "Asia/Shanghai"
```
...
```
📅 设计每日资讯 | Design Daily — 2026-03-31

🤖 AI 动态 / AI Updates

1. We're upgrading our smartest model / 升级我们最智能的模型
   Anthropic releases Claude Opus 4.6, featuring improved coding skills,
   longer agentic task support, and a 1M token context window.
   Anthropic 发布 Claude Opus 4.6，具备更强的编码能力、
   更长时间的任务支持，以及 100 万 token 上下文窗口。
...
—
Sources: 13 web_fetch + 6 web_search | Curated by 大统领 🐾
```
